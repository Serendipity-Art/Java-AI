...
Arrays是JDK提供的专门操作数组的工具类,存储在java.util包中

Arrays.equals比较两个数组是否相等
...
```java
    public static void main(String[] args) {
        int[] arrays1 = {73, 80, 62, 34, 56, 78};
        int[] arrays2 = {73, 80, 62, 34, 56, 78};
        int[] arrays3 = {73, 80, 62, 34, 56, 79};
        System.out.println(Arrays.equals(arrays1, arrays2));
        Arrays.sort(arrays1);
        System.out.println(Arrays.toString(arrays1));
        Arrays.fill(arrays2, 73);
        System.out.println(Arrays.toString(arrays2));
        int array4[] = Arrays.copyOf(arrays3, 10);
...
保存6个货柜中所有商品价格,每个货柜10个商品

使用二维数组来保存数据,二维数组中保存的值是另一个一维数组

二维数组简单理解,一维数组里面保存的值是另一个一维数组

如果一维数组中保存的值是其他数组的首地址,那这种数据结构就是二维数组
...
实际使用,超市卖鸡蛋的货柜分成上,中,下3层,每层又分为6个隔板,每个隔板中的鸡蛋个数等于货柜层数*格挡数.
